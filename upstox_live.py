...
def inject_prz(chain_rows, expiry_date_str, step, spot_price):
    ...
    # 🟢 FIND THE ATM ROW TO CALIBRATE THE SMART IMPLIED RATE
    ...
    for r in chain_rows:
        ...
        # 🟢 ALWAYS OVERWRITE UPSTOX DATA WITH OUR DYNAMIC GREEKS AND IV
    ...
    for r in chain_rows:
        ...
        # 🟢 PRICING & REVERSALS USE THE SMART IMPLIED RATE (live_rate)
    ...
    if len(sorted_vols) > 1:
        ...
        if pct >= 75.0:
            if sec_strike > max_strike:
                return {"strike": max_strike, "state": "WTT", "target_strike": sec_strike, "pct": pct, "val": max_vol}
            else:
                return {"strike": max_strike, "state": "WTB", "target_strike": sec_strike, "pct": pct, "val": max_vol}
...
def calculate_coa(chain_rows, symbol, expiry):
    ...
    if mem['sup_strike'] == 0:
        ...
        new_logs.append(f"{current_time} - Market Open: Support Strong at {sup['strike']}, Resistance Strong at {res['strike']}.")
...
def do_login_flow():
    ...
    print(f"\n  Opening browser for Upstox login...\n  If it does not open, click:\n  {login_url}\n")
    try: webbrowser.open(login_url)
    except Exception: pass

    auth_code = input("  Paste the 'code' value from the URL here: ").strip()
    if not auth_code: sys.exit(1)
...
def options_chain():
    ...
    try:
        ...
        # NATIVE UPSTOX CHAIN FOR INDEXES
        ...
        for item in raw_list:
            ...
            def parse_side_nse(d, v_mult, o_mult):
                ...
                # We return placeholder Greeks here, because inject_prz will FORCE calculate them next.
                return {
                    "ltp": fmt(md.get("ltp")), 
                    "oi": base_oi, 
...
if __name__ == "__main__":
    ...
    if not load_saved_token(): do_login_flow()
