...
def load_saved_token():
    ...
    try:
        ...
        # 🟢 FIX 1: Don't just check the date string. 
        # Check if the token is actually physically there.
        if token:
            _access_token = token
            print(f"✅ Token loaded into RAM. Source Date: {token_doc.get('date')}")
            return True
...
def save_token(token):
    ...
    _access_token = token
    try:
        # Use a localized timestamp for better tracking
        ist = pytz.timezone('Asia/Kolkata')
        now_ist = datetime.now(ist).strftime("%Y-%m-%d %H:%M:%S")
        
        result = sys_col.update_one(
            {"_id": "upstox_auth"},
            {"$set": {
                "access_token": token, 
        ...
        print(f"💾 Token saved to MongoDB successfully at {now_ist}")
        return True
    except Exception as e:
